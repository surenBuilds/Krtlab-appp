...
router.post("/generateLessonContent", async (req: Request, res: Response) => { res.json(await gemini.generateLessonContent(req.body)); });
router.post("/generatePracticeLabTask", async (req: Request, res: Response) => { try { const r = await withRetry(() => getAIClient().models.generateContent({ model: TEXT_MODEL, contents: "Generate practice lab", config: { responseMimeType: "application/json" } })); res.json(JSON.parse(r.text || "{}")); } catch { res.json({ id: "lab-"+Date.now(), steps: [] }); } });
