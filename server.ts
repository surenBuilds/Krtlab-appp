/**
 * KrtLab Server — Growth OS Backend
 */
...
if (isProduction) {
  ...
  startDevServer().catch((err) => { console.error("Failed:", err); process.exit(1); });
}

app.use(errorHandler);

if (isProduction) {
  app.listen(PORT, () => { console.log(`KrtLab Growth OS on http://localhost:${PORT} (production)`); });
}