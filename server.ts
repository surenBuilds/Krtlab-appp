...
// Initialize GoogleGenAI on the server with recommended User-Agent header
...
// 1. Generate Lesson Audio (TTS)
app.post("/api/gemini/generateLessonAudio", async (req, res) => {
  const { lessonText } = req.body;
  if (!lessonText) {
    return res.status(400).json({ message: "lessonText is required" });
  }
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      return res.json({ audio: base64Audio, text: explanationText });
    } catch (error: any) {
      if (isQuotaError(error) && retryCount < maxRetries) {
        retryCount++;
        await new Promise((resolve) => setTimeout(resolve, retryCount * 2000));
        continue;
      }
      console.error("Error in generateLessonAudio server route:", error);
      return res.status(500).json({ message: error.message || "Failed to generate lesson audio" });
    }
  }
});

// 2. Generate Simpler Explanation
app.post("/api/gemini/generateSimplerExplanation", async (req, res) => {
  const { lessonText } = req.body;
  if (!lessonText) {
    return res.status(400).json({ message: "lessonText is required" });
  }
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      console.error("Error in generateSimplerExplanation server route:", error);
      return res.status(500).json({ message: error.message || "Failed to simplify explanation" });
    }
  }
});

// 3. Ask Tutor Question
app.post("/api/gemini/askTutorQuestion", async (req, res) => {
  const { lessonText, question, history } = req.body;
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      const base64Audio = audioResponse.candidates?.[0]?.content?.parts?.[0]?.inlineData?.data;
      return res.json({ audio: base64Audio, text: answerText });
    } catch (error: any) {
      if (isQuotaError(error) && retryCount < maxRetries) {
        retryCount++;
        await new Promise((resolve) => setTimeout(resolve, retryCount * 2000));
      ...
      console.error("Error in askTutorQuestion server route:", error);
      return res.status(500).json({ message: error.message || "Failed to ask tutor" });
    }
  }
});

// 4. Analyze Progress
app.post("/api/gemini/analyzeProgress", async (req, res) => {
  const { lessonId, quizScore, mistakes, questionsAsked } = req.body;
  ...
  - Տրված հարցերի քանակ: ${questionsAsked || 0}
  ...
  try {
    ...
    return res.json({
      level: "medium",
      weakPoints: [],
      recommendation: "Շարունակեք աշխատել նույն ոգով:",
      nextLessonType: "same",
    });
  }
});

// 5. Generate Practice Lab Task
app.post("/api/gemini/generatePracticeLabTask", async (req, res) => {
  const { category, subfieldId, subfieldTitle, level, topic } = req.body;
...
JSON ԿԱՌՈՒՑՎԱԾՔ:
{
  ...
  try {
    ...
    console.error("Error generating lab task on server:", error);
    return res.status(500).json({ message: "Failed to generate practice lab task" });
  }
});

// Sources fallback mappings for Lesson generation
...
// Robust Armenian fallback lesson generator
function getFallbackLesson(category: string, subfield: string, level: number, currentTopic?: string, topicIndex?: number) {
  return {
    ...
    keyConcepts: ["Հիմնարար սկզբունքներ", "Տեսական հիմքեր", "Գործնական կիրառություն"],
    ...
    recommendedReading: [
      {
        ...
        author: "KrtLab Academy",
        description: "Foundational concepts for dynamic progression."
      }
    ],
...
// 6. Generate Lesson Content
app.post("/api/gemini/generateLessonContent", async (req, res) => {
  const { category, subfield, level, literature, previousLessons, currentTopic, topicIndex } = req.body;
  ...
  - Beginner: ${literature.beginner?.map((b: any) => `${b.title} by ${b.author} - ${b.description}`).join("; ") || ""}
  - Intermediate: ${literature.intermediate?.map((b: any) => `${b.title} by ${b.author} - ${b.description}`).join("; ") || ""}
  - Advanced: ${literature.advanced?.map((b: any) => `${b.title} by ${b.author} - ${b.description}`).join("; ") || ""}
...
2. DEDUP: Do NOT repeat core concepts taught in previous levels.
...
JSON STRUCTURE:
{
  ...
  return res.json(getFallbackLesson(category, subfield, level, currentTopic, topicIndex));
});

// 7. Extract Terms from Lesson
app.post("/api/gemini/extractTermsFromLesson", async (req, res) => {
  const { lessonContent } = req.body;
  if (!lessonContent) {
    return res.status(400).json({ message: "lessonContent is required" });
  }
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      console.error("Error extracting terms on server:", error);
      return res.json([]);
    }
  }
});

// 8. Generate Progression Feedback (returns raw base64Audio which client converts using pcmToWav)
app.post("/api/gemini/generateProgressionFeedback", async (req, res) => {
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      return res.json({
        ...
        status,
        messageText,
        base64Audio, // Return raw PCM base64 back to client so it can do client-side pcmToWav
      });
    } catch (error: any) {
      if (isQuotaError(error) && retryCount < maxRetries) {
        retryCount++;
      ...
      console.error("Error in generateProgressionFeedback on server:", error);
      return res.json({
        userId,
        lessonId,
        newLevel,
...
// 9. Chat with Mentor
app.post("/api/gemini/chatWithMentor", async (req, res) => {
  ...
  while (retryCount <= maxRetries) {
    try {
      const response = await ai.models.generateContent({
        model: "gemini-3.5-flash",
        contents: messages.map((m: any) => ({ parts: [{ text: m.text }], role: m.role })),
        config: {
          systemInstruction,
        },
      });
      ...
      console.error("Error in chatWithMentor server route:", error);
      return res.json({ text: "Ցավոք, կապի խնդիր առաջացավ: Խնդրում եմ փորձեք մի փոքր ուշ:" });
    }
  }
});

// 10. Generate Language Placement Test
app.post("/api/gemini/generateLanguagePlacementTest", async (req, res) => {
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      console.error("Error in generateLanguagePlacementTest:", error);
      return res.json([]);
    }
  }
});

// 11. Generate Language Vocabulary
app.post("/api/gemini/generateLanguageVocabulary", async (req, res) => {
  const { language, level, count } = req.body;

  const prompt = `Գեներացրու ${count || 20} բառ ${language} լեզվով ${level} մակարդակի համար:
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      console.error("Error in generateLanguageVocabulary:", error);
      return res.json([]);
    }
  }
});

// 12. Generate Standalone Game
app.post("/api/gemini/generateStandaloneGame", async (req, res) => {
  const { topic, level, domain, content } = req.body;
...
`;
  ...
  try {
    ...
    console.error("Error in generateStandaloneGame:", error);
    return res.status(500).json({ message: "Failed to generate standalone game" });
  }
});

// 13. Generate Language Grammar
app.post("/api/gemini/generateLanguageGrammar", async (req, res) => {
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      console.error("Error in generateLanguageGrammar:", error);
      return res.status(500).json({ message: "Failed to generate language grammar" });
    }
  }
});

// 14. Explain Quiz Mistake
app.post("/api/gemini/explainQuizMistake", async (req, res) => {
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      return res.json({ text: response.text || "Ներողություն, չհաջողվեց բացատրություն գտնել:" });
    } catch (error: any) {
      if (isQuotaError(error) && retryCount < maxRetries) {
        retryCount++;
        await new Promise((resolve) => setTimeout(resolve, retryCount * 2000));
      ...
      console.error("Error in explainQuizMistake route:", error);
      return res.status(500).json({ message: "Failed to explain quiz mistake" });
    }
  }
});

// 15. Generate Certificate Outcomes
app.post("/api/gemini/generateCertificateOutcomes", async (req, res) => {
  const { courseName, levelName } = req.body;

  const prompt = `Generate a professional 2-sentence summary of learning outcomes for a student who completed the course "${courseName}" at level "${levelName}" on the KrtLab educational platform. Focus on skills, knowledge, and competencies achieved. Be concise and professional.`;
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      return res.json({ text: response.text || "Successfully demonstrated comprehensive knowledge and practical application of core principles within the subject area." });
    } catch (error: any) {
      if (isQuotaError(error) && retryCount < maxRetries) {
        retryCount++;
        await new Promise((resolve) => setTimeout(resolve, retryCount * 2000));
      ...
      console.error("Error in generateCertificateOutcomes route:", error);
      return res.status(500).json({ message: "Failed to generate certificate outcomes" });
    }
  }
});

// 16. Run Optimization Engine
app.post("/api/gemini/runOptimizationEngine", async (req, res) => {
  const { levelData, performance } = req.body;
...
JSON SCHEMA:
{
  ...
  "improvedData": { ... updated Level object ... }
}
...
`;
  ...
  try {
    ...
    console.error("Optimization Engine failed on server:", error);
    return res.status(500).json({ message: "Failed to optimize content" });
  }
});

// Mount Vite middleware / production handlers
async function startViteAndStatic() {
