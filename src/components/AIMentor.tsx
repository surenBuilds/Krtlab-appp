...
export const AIMentor: React.FC<AIMentorProps> = ({ userName, isOpen, onClose }) => {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'ai', text: string }[]>([
    { role: 'ai', text: `Ողջույն, ${userName}: Ես KrtLab AI մենթորն եմ: Ինչո՞վ կարող եմ օգնել:` }
  ]);
  ...
  const contextStr = useMemo(() => {
    ...
    const baseline = diagnoseSkills(gp);
    const skillGraph = buildSkillGraph(gp);
    const nextAction = computeNextAction(skillGraph, []);
    const top = baseline.filter(b => b.masteryScore > 0).slice(0, 8);
    const strengths = top.filter(b => b.masteryScore > 50).map(b => b.name);
    const weaknesses = top.filter(b => b.masteryScore > 0 && b.masteryScore < 30).map(b => b.name);
    const skillsSummary = top.map(b => `${b.name}:${b.masteryScore}%`).join(', ');
    return `[FULL CONTEXT]
Name: ${profile.name} | Level: ${profile.level||1} | XP: ${profile.xp||0} | Streak: ${profile.streak||0}d
Goal: ${gp?.discovery?.goal||'not set'} | Style: ${gp?.discovery?.style||'not set'}
Skills: ${skillsSummary}
Strengths: ${strengths.join(', ')||'none'}
Weaknesses: ${weaknesses.join(', ')||'none'}
Next Action: ${nextAction.reason} → ${nextAction.suggestedTask}
[TASK: Give 1-3 specific, actionable recommendations in Armenian]`;
  ...
  return <>
    ...
    <div className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white shadow-2xl z-[70] flex flex-col border-l">
