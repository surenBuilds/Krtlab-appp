...
export const GrowthDashboard: React.FC = () => {
  ...
  // Read goals from persistent state — useState init from localStorage
  ...
  useEffect(() => {
    if (profile?.intelligenceState?.goals?.length) setGoals(profile.intelligenceState.goals);
  ...
  const nextAction = useMemo(() => computeNextAction(skillGraph, domainGoals), [skillGraph, domainGoals]);
  const mission = useMemo(() => generateDailyMission(skillGraph, domainGoals, gp), [skillGraph, domainGoals, profile]);

  const topSkills = useMemo(
    () => baseline.filter(b => b.masteryScore > 0).slice(0, showAllSkills ? 20 : 6),
    [baseline, showAllSkills]
  );
  const strengths = useMemo(
    () => baseline.filter(b => b.masteryScore > 50).map(b => b.name).slice(0, 5),
    [baseline]
  );
  const weaknesses = useMemo(
    () => baseline.filter(b => b.masteryScore > 0 && b.masteryScore < 30).map(b => b.name).slice(0, 5),
    [baseline]
  );
  ...
  return <div className="space-y-8">
    ...
    <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-[3rem] p-10 text-white shadow-2xl shadow-slate-300">
      <div className="flex items-start justify-between flex-wrap gap-6">
        <div>
          <p className="text-slate-400 text-sm font-bold uppercase tracking-wider mb-2">Your Mission</p>
          <h1 className="text-4xl font-black tracking-tight">Growth Dashboard</h1>
          <p className="text-slate-400 mt-3 text-lg">
            {goals.length > 0
              ? `Working toward: ${goals[0].title}`
              : "Set a goal to personalize your path"}
          </p>
        </div>
        <div className="flex gap-4">
          ...
          <div className="text-center bg-white/10 rounded-2xl p-4">
            <div className="text-3xl font-black">Lv.{gp?.level || 1}</div>
      ...
      <div className="lg:col-span-2 bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-black flex items-center gap-2">
            <BrainCircuit size={20}/>Skill Graph
          </h3>
          <button onClick={() => setShowAllSkills(!showAllSkills)} className="text-sm text-primary font-bold">
            {showAllSkills ? "Show less" : `View all ${baseline.length} skills`}
          </button>
        </div>
          ...
          {topSkills.length > 0 ? topSkills.map(skill => (
            <div key={skill.skillId}>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs text-slate-400">{skill.masteryScore}%</span>
              </div>
              <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
                <motion.div
                  ...
                  className={cn("h-full rounded-full",
                    skill.masteryScore > 70 ? "bg-emerald-500" :
                    skill.masteryScore > 40 ? "bg-primary" : "bg-amber-500"
                  )}
                />
              </div>
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className={cn("text-[10px] font-bold",
                  skill.confidence === "high" ? "text-emerald-600" :
                  skill.confidence === "medium" ? "text-amber-600" : "text-red-500"
                )}>{skill.confidence}</span>
              </div>
            </div>
          )) : (
            <div className="text-center py-8 text-slate-400">
              <p className="text-lg font-bold mb-2">No skills assessed yet</p>
              <p className="text-sm">Complete lessons or set a goal to see your skill graph</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-black flex items-center gap-2">
          <Zap size={20} className="text-accent"/>Today's Mission
          {goals.length > 0 && (
            <span className="text-sm font-normal text-slate-400 ml-2">— {goals[0].title}</span>
          )}
        </h3>
        ...
        {mission.tasks.map((task, i) => (
          <motion.div
            ...
            className={cn("p-6 rounded-2xl border-2 transition-all cursor-pointer hover:shadow-lg",
              task.type === "learn" ? "bg-blue-50 border-blue-100" :
              task.type === "practice" ? "bg-amber-50 border-amber-100" :
              task.type === "build" ? "bg-emerald-50 border-emerald-100" :
    ...
    {nextAction && (
      <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-lg font-black">AI Recommendation</h4>
            <p className="text-slate-500 mt-1">{nextAction.reason}</p>
            <p className="text-primary font-bold mt-2 flex items-center gap-1">
              {nextAction.suggestedTask}<ArrowRight size={14}/>
            </p>
          </div>
        </div>
      </div>
